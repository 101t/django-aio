...
class NotificationRequestAdmin(admin.ModelAdmin):
	...
	def render_change_form(self, request, context, *args, **kwargs):
		context['adminform'].form.fields['user'].queryset = User.objects.exclude(role=User.STUDENT)
		context['adminform'].form.fields['users'].queryset = User.objects.filter(role=User.STUDENT)
