...
class NotificationRequestForm(forms.ModelForm):
    ...
    selectall = forms.BooleanField(required=False, label=_("Select all users"),
                                   help_text=_("Send this notification to all users in system."))
    ...
    def clean(self):
        if self.cleaned_data["selectall"]:
            # if all users selected will modify form user field to all users ids
            self.cleaned_data["users"] = User.objects.filter(is_active=True).filter(is_verified=True).values_list("pk",
                                                                                                                  flat=True)
